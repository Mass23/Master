...
with open("snipre_input.csv", "w") as f:
    ...
    for gene in range(0, columns_number):
        ...
        results_line = "\t".join([str(gene_results.gene_id), str(gene_results.pr), str(gene_results.fr), str(gene_results.ps), str(gene_results.fs), str(gene_results.trepl), str(gene_results.tsil), nout, npop + "\n"])
